...
class HostedChannelTypesSpec extends AnyFunSuite {
  ...
  val initHostedChannel: InitHostedChannel = InitHostedChannel(maxHtlcValueInFlightMsat = UInt64(90000L), htlcMinimumMsat = 10 msat, maxAcceptedHtlcs = 3, 1000000L msat, 5000, 1000000 sat,
    initialClientBalanceMsat = 0 msat, ByteVector.empty)
  ...
  val localCommitmentSpec: CommitmentSpec = CommitmentSpec(htlcs = Set.empty, feeratePerKw = FeeratePerKw(Satoshi(0L)), lcss1.localBalanceMsat, lcss1.remoteBalanceMsat)
  ...
  test("Processing HTLCs") {
    ...
    val Failure(_: HtlcValueTooSmall) = hdc.sendAdd(cmdAdd1, Local(UUID.randomUUID, None), blockHeight = 100)
    val (_, cmdAdd2) = makeCmdAdd(50 msat, randomKey.publicKey, currentBlockHeight = 100)
    val Failure(_: ExpiryTooSmall) = hdc.sendAdd(cmdAdd2, Local(UUID.randomUUID, None), blockHeight = 300)
    val (_, cmdAdd3) = makeCmdAdd(50000 msat, randomKey.publicKey, currentBlockHeight = 100)
    val Success((hdc1, _)) = hdc.sendAdd(cmdAdd3, Local(UUID.randomUUID, None), blockHeight = 100)
    ...
    val Success((hdc2, _)) = hdc1.sendAdd(cmdAdd4, Local(UUID.randomUUID, None), blockHeight = 100)
    ...
    val Failure(InsufficientFunds(_, _, missing, _, _)) = hdc2.sendAdd(cmdAdd5, Local(UUID.randomUUID, None), blockHeight = 100)
    assert(missing === 10.sat)
    val (_, cmdAdd6) = makeCmdAdd(90001 msat, randomKey.publicKey, currentBlockHeight = 100)
    val Failure(_: HtlcValueTooHighInFlight) = hdc.sendAdd(cmdAdd6, Local(UUID.randomUUID, None), blockHeight = 100)
    ...
    val Success((hdc3, bob2AliceAdd)) = hdc.sendAdd(cmdAdd7, Local(UUID.randomUUID, None), blockHeight = 100)
    val Success((hdc4, _)) = hdc3.sendAdd(cmdAdd8, Local(UUID.randomUUID, None), blockHeight = 100)
    val Success((hdc5, _)) = hdc4.sendAdd(cmdAdd9, Local(UUID.randomUUID, None), blockHeight = 100)
    val Failure(_: TooManyAcceptedHtlcs) = hdc5.sendAdd(cmdAdd10, Local(UUID.randomUUID, None), blockHeight = 100)
