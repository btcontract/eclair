...
class HostedChannelGateway(nodeParams: NodeParams, router: ActorRef, relayer: ActorRef)(implicit ec: ExecutionContext = ExecutionContext.Implicits.global) extends Actor with ActorLogging {
  ...
  context.system.scheduler.scheduleWithFixedDelay(6.hours, 6.hours, self, CMD_HOSTED_REMOVE_IDLE_CHANNELS)
...
object HostedChannelGateway {
  ...
  def props(nodeParams: NodeParams, router: ActorRef, relayer: ActorRef): Props = Props(new HostedChannelGateway(nodeParams, router, relayer))
