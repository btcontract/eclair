...
import fr.acinq.eclair.wire.CommonCodecs.{bytes32, publicKey, uint64overflow, setCodec}
...
object HostedChannelCodecs {
  val HOSTED_DATA_COMMITMENTS_Codec: Codec[HOSTED_DATA_COMMITMENTS] = {
    ("remoteNodeId" | publicKey) ::
      ...
      ("failedToPeerHtlcLeftoverIds" | setCodec(uint64overflow)) ::
      ("fulfilledByPeerHtlcLeftoverIds" | setCodec(uint64overflow)) ::
